...
def test_sfstp():
    ...
    for key, value in profiles.profiles.items():
        ...
        for series, series_value in value[0].model.fit.items():
            reg_params_dict["Model_str"].append(key)
            reg_params_dict["Series_str"].append("Series " + str(series))
            if "Intercept" in series_value.params:
                reg_params_dict["Intercept"].append(series_value.params["Intercept"])
            else:
                reg_params_dict["Intercept"].append(0)
            reg_params_dict["Slope"].append(series_value.params["x"])
            if "I(x ** 2)" in series_value.params:
                reg_params_dict["Quad. term."].append(series_value.params["I(x ** 2)"])
            else:
