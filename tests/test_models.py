...
class TestModelsManager:
    ...
    def test_initialize_models_multiple(self, test_modelsmanager_hardcoded):
        ...
        assert test_modelsmanager_hardcoded.initialized_models_list == [
            "Linear",
            "Quadratic",
        ]
    ...
    def test_initialize_models_all(self, test_modelsmanager_hardcoded):
        ...
        assert test_modelsmanager_hardcoded.initialized_models_list == list(
            test_modelsmanager_hardcoded.get_available_models().keys()
        )
    ...
    def test_get_model_weight(self, test_modelsmanager_hardcoded):
        assert (
            type(test_modelsmanager_hardcoded.get_model_weight("1/X Weighted Linear"))
            == str
        )
        with pytest.warns(UserWarning):
